...
const button = `inline-flex min-h-[44px] items-center justify-center gap-2 rounded-lg px-4 text-sm font-semibold transition-colors ${focusRing}`;
...
const tertiary = `inline-flex min-h-[44px] items-center gap-1.5 rounded-lg px-2 text-sm font-semibold text-ink-secondary underline-offset-2 hover:text-ink hover:underline ${focusRing}`;
...
function TrustChip({ tone, text }: { tone: TrustTone; text: string }) {
  ...
  return (
    <p className={`flex items-start gap-2 rounded-lg px-3 py-2 text-sm font-medium ${box}`}>
...
export const ReviewCard = forwardRef<HTMLLIElement, ReviewCardProps>(function ReviewCard(props, ref) {
    ...
    return (
      <li ref={ref} data-review-card={item.id} className="flex shrink-0 items-center gap-3 rounded-xl border border-border bg-paper/60 py-1 pl-3 pr-1">
        ...
        <div className="min-w-0 flex-1 py-2">
          ...
          <p className="truncate text-sm text-ink-secondary">{snippetOf(entry, blockText)}</p>
        </div>
    ...
    return (
      <li ref={ref} data-review-card={item.id} className="shrink-0">
        <button
          ...
          className={`flex min-h-[44px] w-full items-center gap-3 rounded-xl border border-border bg-surface py-2 pl-3 pr-3 text-left hover:bg-paper-deep ${focusRing}`}
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-sm text-ink">{snippetOf(entry, blockText)}</span>
          </span>
  ...
  return (
    <li
      ...
      aria-current="true"
      className="flex shrink-0 flex-col gap-3 rounded-xl border border-accent/60 bg-surface p-4 shadow-sm focus:outline-none"
    >
      <div className="flex items-baseline justify-between gap-2">
        <h3 className="text-sm font-semibold text-ink">{full}</h3>
    ...
    <>
      ...
      {suggested !== null && (
        <div className="rounded-lg bg-success-soft p-3">
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-success">Suggested</p>
      ...
      {original && (
        <div className="rounded-lg border border-border bg-paper-deep/60 p-3">
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-ink-secondary">Your original</p>
          <p className="whitespace-pre-wrap break-words text-sm leading-relaxed text-ink">
            {suggested !== null ? (
    ...
    <>
      <div className="flex flex-col gap-1">
        ...
        <textarea
          ...
          className={`w-full resize-y rounded-lg border border-border-strong bg-surface p-3 text-base leading-relaxed text-ink ${focusRing}`}
        />
