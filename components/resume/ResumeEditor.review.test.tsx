...
const chip = () => screen.getByRole("button", { name: /to review|All clear|Checking|^Review$/ });

describe("ResumeEditor review wiring", () => {
  it("chip count equals the highlighted passages, and a free user can fix spelling with undo/redo", async () => {
    const { container } = renderEditor();
    expect(chip()).toHaveTextContent(/Checking/);

    // typo (warn) + unverified 65% (verify)
    await waitFor(() => expect(chip()).toHaveTextContent("2 to review, 1 to verify"), { timeout: 5000 });
    expect(container.querySelectorAll("mark")).toHaveLength(2);

    fireEvent.click(chip());
    const panel = screen.getByRole("dialog", { name: "Review" });
    expect(panel).toBeInTheDocument();
    // Opens on the urgent verify item.
    expect(screen.getByText(/Not in your profile: 65%/)).toBeInTheDocument();
    expect(screen.queryByText(/Upgrade to See Mistakes/)).toBeNull();

    fireEvent.click(screen.getByRole("tab", { name: /Fixes/ }));
    expect(screen.getByText(/Possible spelling mistake: 'recieved'/)).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Fix" }));
    await waitFor(() => expect(chip()).toHaveTextContent("1 to review, 1 to verify"), { timeout: 5000 });
  ...
  it("dismissing a card drops the count and highlight, and Restore brings it back", async () => {
    const { container } = renderEditor();
  ...
  it("Revert restores the profile wording through History; Accept on a new claim persists across a reload", async () => {
