...
\`\`\``;
    ...
    const result = parseScoreResponse(raw, "simulation", undefined, 45);
  ...
  it("never returns star_scores for a coaching-mode response, even if the model hallucinates one", () => {
    ...
    const result = parseScoreResponse(raw, "coaching", undefined, 30);
  ...
  it("falls back to defaults without throwing on truncated/invalid JSON", () => {
    ...
    const result = parseScoreResponse(truncated, "simulation", "typed fallback answer", 0);
  ...
  it("falls back to defaults without throwing on completely empty text", () => {
    const result = parseScoreResponse("", "simulation", undefined, 10);
  ...
  it("clamps out-of-range star scores into 1-5 and coerces missing fields", () => {
    ...
    const result = parseScoreResponse(raw, "simulation", undefined, 20);
    expect(result.star_scores).toEqual({
      situation: 5,
      task: 1,
      action: 3,
  ...
  it("handles a non-object JSON payload (e.g. a bare array) without throwing", () => {
    const raw = "[1, 2, 3]";
    const result = parseScoreResponse(raw, "simulation", "fallback text", 15);
